...
defmodule Fd.Test do
    ...
    test "next", kvs do
        ...
        _r0 = KVS.reader(id: rid) = :kvs.save(:kvs.top(:kvs.reader("/crm/luck")))
        kvs[:id1] |> Enum.with_index 
                  |> Enum.each(fn {_id,9} ->
                r = :kvs.load_reader(rid)
                r01 = :kvs.next(r)
                assert r1 = KVS.reader(feed: "//crm/luck", cache: _c1, count: 10, dir: 0, args: [^last]) = r01
                assert KVS.reader(args: [], feed: "//crm/luck", cache: _c2) = :kvs.save(r1)
            {_id,i} -> 
    ...
    test "prev", kvs do
        ...
        ids |> Enum.with_index
            |> Enum.each(fn {_id,9} ->
                ...
                assert KVS.reader(args: [], feed: "//crm/luck", cache: _c1) = :kvs.save(r1)
            {_id,i} ->
                r = :kvs.load_reader(rid)
        ...
        r = :kvs.load_reader(rid)
        assert r1 = :kvs.prev(r)
        assert _r2 = KVS.reader(:kvs.prev(:kvs.top(r1)), args: [])
    end
    ...
    test "cut the *uck", _kvs do
        ...
        all2 = :kvs.all("/crm")
        assert 10 = length(all2)
        assert ^all2 = :kvs.all("/crm/truck")

        :kvs.cut("/crm/truck")

        all3 = :kvs.all("/crm")
        assert 0 = length(all3)
    end

    test "remove the *uck with readers", _kvs do
